  ...
  renderDots(marking) {
    ...
    if (marking.dots && Array.isArray(marking.dots) && marking.dots.length > 0) {
      ...
      return validDots.map((dot, index) => {
        return React.createElement(View, { key: dot.key ? dot.key : index, style: [baseDotStyle, { backgroundColor: marking.selected && dot.selectedDotColor ? dot.selectedDotColor : dot.color }] });
      });
    }
    return;
  }
